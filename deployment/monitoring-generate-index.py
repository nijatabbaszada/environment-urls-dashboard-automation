...
<title>Monitoring Stack Environments</title>
<style>
...
.card h3 {margin:0;font-size:22px;color:#000;}
...
.copy-icon {
  opacity:0;position:absolute;right:-32px;top:50%;transform:translateY(-50%);
...
.copied {
  display:none;position:absolute;right:-75px;top:48%;transform:translateY(-50%);
  background:#4CAF50;color:white;padding:3px 8px;border-radius:6px;font-size:13px;
}

.home-button {
  position:absolute;left:30px;top:50%;transform:translateY(-50%);
  display:inline-flex;align-items:center;background-color:#ffffff22;border:1px solid #ffffff44;
  color:white;padding:8px 14px;border-radius:8px;text-decoration:none;font-size:16px;
  font-weight:500;transition:0.25s ease;
}
.home-button:hover {background-color:#ffffff33;transform:translateY(-55%) scale(1.05);}
</style>
</head>
<body>
<header>
  <a href="index.html" class="home-button">🏠 Back</a>
  <h1>Monitoring Stack Environments</h1>
</header>
<div class="container">
"""
...
© 2025 Monitoring Stack Dashboard | Open Source Edition
</footer>
</body>
</html>
"""
...
if __name__ == "__main__":
    ...
    monitoring_dir = os.path.join(root_dir, "..", "monitoring-stack")

    cards = ""

    for f in ["prod.yaml", "preprod.yaml", "develop.yaml"]:
        path = os.path.join(monitoring_dir, f)
    ...
    output_path = os.path.join(root_dir, "monitoring-index.html")
